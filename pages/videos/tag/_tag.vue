<template>
  <div v-if="!$fetchState.pending && !$fetchState.error" class="content-wrapper writings">
    ...
    <main id="main-content">
      <heading :breadcrumb="true" titletag="h1" :title="titleData" :uppercase="true" />
      ...
      <div v-else class="videos-list">
        <div class="videos">
          <video-card v-for="video in videosData" :key="video.slug" :video="video" />
        </div>
      </div>
    </main>
  </div>
...
export default defineComponent({
  setup() {
    ...
    useFetch(async () => {
      const { videos, tags, tag, title } = await useContent(route.value.params.tag);
